...
class Cache
{
    ...
    static function resolve_url($url, $protocol, $host, $base_path, Dompdf $dompdf)
    {
        ...
        $is_data_uri = strpos($parsed_url['protocol'], "data:") === 0;
        ...
        try {
            ...
            if (strpos($full_url, "://") === false && !$is_data_uri) {
                $resolved_url = $full_url;
            }

            if (!isset($resolved_url)) {
                ...
                self::addTempImage($full_url, $resolved_url);
            ...
            if ($type === "svg") {
                ...
                xml_set_element_handler(
                    $parser,
                    function ($parser, $name, $attributes) use ($dompdf, $parsed_url, $full_url) {
                        if ($name === "image") {
                            ...
                            if (!empty($url)) {
                                ...
                                [$resolved_url, $type, $message] = self::resolve_url($url, $parsed_url["protocol"], $parsed_url["host"], $parsed_url["path"], $dompdf);
                                if (!empty($message)) {
